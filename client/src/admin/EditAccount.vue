    ...
    <div class="container w-50 p-3 my-1 bg-light border border-info">
      <form class="was-validated" @submit.prevent="updateAccount">
        <div class="register">
          ...
          <input
            type="text"
            v-model="model.account.login_name"
            class="form-control"
            id="lblogin"
            disabled
          />
          ...
          <input
            type="text"
            id="lbdisplay"
            v-model="model.account.display_name"
          ...
          <div>
            ...
            <input type="checkbox" @change="gererPassword()" />

            <input
              type="password"
              v-model="model.account.pass_word"
              placeholder="Enter Password"
              id="lbpassword"
              :disabled="disablePassword == true"
              class="form-control"
              :required="disablePassword == false"
            />
          </div>
...
export default {
  ...
  methods: {
    gererPassword() {
      if (this.disablePassword) this.disablePassword = false;
      else this.disablePassword = true;
    },
    ...
    async updateAccount() {
      try {
        ...
        if(this.disablePassword == false){
         url = urlacc.getEditPWDelAccUrl(this.model.account.id);
      ...
      {
        url = urlacc.getEditDelAccUrl(this.model.account.id);
         result = await axios.put(url, {
          display_name: this.model.account.display_name,
          role_id: this.model.account.role_id,
        });
